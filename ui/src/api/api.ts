...
export async function getAgents(
  ...
  return get<GetAgentsResponse>("/agents", undefined, { signal });
}
...
export async function getChatSessions(
  ...
  return get<GetChatSessionsResponse>("/chat-sessions", undefined, { signal });
}
...
export async function getChatSessionsByAgentId(
  ...
  return get<GetChatSessionsResponse>(
    `/chat-sessions/agent/${agentId}`,
    undefined,
    {
      signal,
    },
...
export async function getKnowledgeBases(
  signal?: AbortSignal,
): Promise<GetKnowledgeBasesResponse> {
  return get<GetKnowledgeBasesResponse>("/knowledge-bases", undefined, {
    signal,
  });
}
...
export async function getDocumentsByKbId(
  ...
  return get<GetDocumentsResponse>(`/documents/kb/${kbId}`, undefined, {
    signal,
  });
}
...
export async function uploadDocument(
  ...
  const response = await fetchWithAuth(`${BASE_URL}/documents/upload`, {
    method: "POST",
    body: formData,
    signal,
  });
  const apiResponse = await handleResponse<CreateDocumentResponse>(response);
...
export async function getOptionalTools(
  signal?: AbortSignal,
): Promise<GetOptionalToolsResponse> {
  const tools = await get<ToolVO[]>("/tools", undefined, { signal });
  return { tools };
}
